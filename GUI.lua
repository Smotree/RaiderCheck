...
function RaiderCheck:CreatePlayerFrame(playerName, index)
	...
	if hasAddon then
		...
		-- Дополнительная строка под зачарованиями (пустые сокеты и старые камни)
		...
		if gearAnalysis.hasData then
			...
			-- Устаревшие камни
			local oldGems = gearAnalysis.lowQualityGemsCount
			if not oldGems and gearAnalysis.lowQualityGems then
				oldGems = #gearAnalysis.lowQualityGems
			end
			if oldGems and oldGems > 0 then
				table.insert(extraParts, "Старые: " .. oldGems)
			end
			...
			gearExtraText:SetTextColor(0.6, 0.6, 0.6, 1)
		end
		...
		if hasExtra then
			talentText:SetPoint("TOPLEFT", 40, -68)
		else
			talentText:SetPoint("TOPLEFT", 40, -52)
		end
...
function RaiderCheck:ShowPlayerDetails(playerName)
	...
	if talentAnalysis.hasData then
		...
		-- Краткое summary по устаревшим камням рядом со спеком
		local oldGems = gearAnalysisForGems.lowQualityGemsCount
		if not oldGems and gearAnalysisForGems.lowQualityGems then
			oldGems = #gearAnalysisForGems.lowQualityGems
		end
		if oldGems and oldGems > 0 then
			print("|cFFFF0000Устаревшие камни:|r " .. oldGems)
		end
	end

	-- Проверка устаревших камней (новая система качества)
	local gearAnalysisForGems = self:AnalyzePlayerGear(playerName)
	if
		gearAnalysisForGems.hasData
		and gearAnalysisForGems.lowQualityGems
		and #gearAnalysisForGems.lowQualityGems > 0
	then
		print("|cFFFF0000Устаревшие камни:|r")
		for _, gemInfo in ipairs(gearAnalysisForGems.lowQualityGems) do
